...
if artist_input != "":
    ...
    if artist_index != None:
        ...
        # the line chart shows the relationships more clearly compared to area chart
        st.line_chart(tracks_df, x="Track Name", y=["Popularity", "Danceability", "Energy", "Valence", "Liveness"],
                      color=["#43B3FF", "#A020F0", "#FF69B4", "#2CE5C3", "#1F53E0"], width=800, height=500)

        # another line chart option with point markers and y-axis label
        ...
        # could be an area chart, but it seems inaccurate?
        st.area_chart(tracks_df, x="Track Name", y=["Popularity", "Danceability", "Energy", "Valence", "Liveness"],
                      color=["#43B3FF", "#A020F0", "#FF69B4", "#2CE5C3", "#1F53E0"], width=800, height=500)

        # area chart with plotly
        fig = px.area(tracks_df, x="Track Name", y=["Popularity", "Danceability", "Energy", "Valence", "Liveness"],
                      color_discrete_sequence=["#43B3FF", "#A020F0", "#FF69B4", "#2CE5C3", "#1F53E0"])
        fig.update_layout(width=800, height=500, yaxis_title="Rating")
        st.plotly_chart(fig)